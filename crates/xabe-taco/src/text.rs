...
///
/// Full-width punctuation is folded to ASCII on the way through. That is the
/// same class of bug as the diacritics and it was found the same way: the mark
/// is not in the alphabet, so it vanishes without a word.
pub fn poj_to_tlpa(text: &str) -> String {
    ...
    for c in text.chars() {
        if syllabic(c) {
            ...
            out.push(ascii_punct(c).unwrap_or(c));
        }
    }
    if !buf.is_empty() {
